...
# Ask Doubt on telegram @Razer358
...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
            ...
            InlineKeyboardButton('📢 Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ', url="https://t.me/Razer_supportx"),
            InlineKeyboardButton('🔰 Mᴏᴠɪᴇ Gʀᴏᴜᴘ', url='https://t.me/Razer_bots')
        ],[
    ...
    if len(message.command) != 2:
        ...
        await asyncio.sleep(1)
        await m.delete()
        m=await message.reply_sticker("CAACAgQAAxkBAAEL-AJmJhk2-DErPfWiRs-q_gXrs9UqugAC0wIAAnJxFyWzUIifR7T3HTQE") 
        await asyncio.sleep(1)
        await m.delete()
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                        ...
                        InlineKeyboardButton('𝗕𝗢𝗧 𝗢𝗪𝗡𝗘𝗥', url="https://t.me/Razer358")
                    ],[
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            if STREAM_MODE == True:
                button = [[
                    InlineKeyboardButton('🚀 Fast Download / Watch Online🖥️', callback_data=f'generate_stream_link:{file_id}') #Don't change anything without contacting me @Razer358
                ]]
            else:
                button = [[
                    InlineKeyboardButton('Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ', url=f'https://t.me/{SUPPORT_CHAT}'),
    ...
    if not files_:
        ...
        try:
            ...
            title = '@Razer_bots  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
            ...
            k = await msg.reply("<b><u>♻️♻️♻️IMPORTANT♻️♻️♻️</u></b>\n\nThis Movie File/Video will be deleted in <b><u>10 mins</u> 🫥 <i></b>(Due to Copyright Issues)</i>.\n\n<b><i>Please forward this File/Video to your Saved Messages and Start Download there</i></b>",quote=True)
            await asyncio.sleep(600)
            await msg.delete()
    ...
    files = files_[0]
    title = '@Razer_bots  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@Razer_bots  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
                InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
